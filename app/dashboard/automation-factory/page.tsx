...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("automationFactory"), alt: "Automation" },
  { src: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&q=80", alt: "Workflow" },
  { src: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=400&q=80", alt: "Integration" },
];
...
export default function AutomationFactoryPage() {
  ...
  const suggestions = getSuggestionsForFactory("automationFactory");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "automationFactory", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Create automation workflows: ideas, integrations, Zapier/Make flows, webhooks, automation steps.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Workflow or integration idea</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Lead capture workflow, CRM, email automation"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="automationFactory"
              factoryName="AI Automation Factory"
              prompt={prompt}
              resultData={result}
