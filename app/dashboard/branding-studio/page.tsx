...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("brandingStudio"), alt: "Brand" },
  { src: "https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=400&q=80", alt: "Identity" },
  { src: "https://images.unsplash.com/photo-1561070791-2526d30994b5?w=400&q=80", alt: "Style" },
];
...
export default function BrandingStudioPage() {
  ...
  const suggestions = getSuggestionsForFactory("brandingStudio");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "brandingStudio", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Branding Studio Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate full brand identity: logo concepts, color palettes, typography, social media style.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Brand or business description</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Full brand identity, logo, colors, typography"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="brandingStudio"
              factoryName="AI Branding Studio Factory"
              prompt={prompt}
              resultData={result}
