...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("courseCreator"), alt: "Course" },
  { src: "https://images.unsplash.com/photo-1501504905252-473c47e087f8?w=400&q=80", alt: "Learning" },
  { src: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=400&q=80", alt: "Education" },
];
...
export default function CourseCreatorPage() {
  ...
  const suggestions = getSuggestionsForFactory("courseCreator");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "courseCreator", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Create online courses: title, outline, lesson structure, video script ideas, marketing plan.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Course topic or type</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Online course outline, modules, lessons"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="courseCreator"
              factoryName="AI Course Creator"
              prompt={prompt}
              resultData={result}
