...
import { ArrowLeft, Layout, Loader2, Sparkles } from "lucide-react";
...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("websiteTemplates"), alt: "Website template" },
  { src: "https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=400&q=80", alt: "Layout" },
  { src: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&q=80", alt: "Preview" },
];
...
export default function WebsiteTemplatesPage() {
  ...
  const suggestions = getSuggestionsForFactory("websiteTemplates");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "websiteTemplates", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Website Templates Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate ready-made website templates with hero, services, about, and contact sections.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Business type or template description</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Restaurant website, hero, menu, contact"
        ...
        <AnimatePresence>
          {loading && <FactorySkeleton lines={3} className="mb-8" />}
        </AnimatePresence>
        ...
        {!loading && result != null && (
          ...
          >
            ...
            <ResultToolbar
              factoryId="websiteTemplates"
              factoryName="AI Website Templates Factory"
              prompt={prompt}
              resultData={result}
