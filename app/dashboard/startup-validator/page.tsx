...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("startupValidator"), alt: "Startup" },
  { src: "https://images.unsplash.com/photo-1552664730-d307ca884978?w=400&q=80", alt: "Validation" },
  { src: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=400&q=80", alt: "Analysis" },
];
...
export default function StartupValidatorPage() {
  ...
  const suggestions = getSuggestionsForFactory("startupValidator");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "startupValidator", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Analyze startup ideas: market analysis, competitors, opportunity score, risks, recommendations.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Startup idea to validate</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Validate SaaS startup idea, market, competitors"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="startupValidator"
              factoryName="AI Startup Validator"
              prompt={prompt}
              resultData={result}
