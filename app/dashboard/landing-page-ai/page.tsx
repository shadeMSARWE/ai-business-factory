...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("landingPageAI"), alt: "Landing" },
  { src: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&q=80", alt: "Page" },
  { src: "https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=400&q=80", alt: "CTA" },
];

export default function LandingPageAIPage() {
  const [prompt, setPrompt] = useState("");
  const [result, setResult] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const suggestions = getSuggestionsForFactory("landingPageAI");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "landingPageAI", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Generate high-converting landing pages with headline, features, testimonials, and CTA.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Product or page type</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Product launch, headline, CTA"
        ...
        {!loading && result != null && (
          ...
          >
            ...
            <ResultToolbar
              factoryId="landingPageAI"
              factoryName="AI Landing Page Generator"
              prompt={prompt}
              resultData={result}
