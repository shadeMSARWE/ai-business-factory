...
import { ArrowLeft, Smartphone, Loader2, Sparkles } from "lucide-react";
...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("appBuilder"), alt: "App" },
  { src: "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=400&q=80", alt: "Mobile" },
  { src: "https://images.unsplash.com/photo-1551650975-87deedd944c3?w=400&q=80", alt: "UI" },
];
...
export default function AppBuilderPage() {
  ...
  const suggestions = getSuggestionsForFactory("appBuilder");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "appBuilder", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI App Builder Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate mobile and web app structure with features, UI layout, and screen structure.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">App type or description</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Fitness app, workout tracking"
        ...
        {!loading && result != null && (
          ...
          >
            ...
            <ResultToolbar
              factoryId="appBuilder"
              factoryName="AI App Builder Factory"
              prompt={prompt}
              resultData={result}
