...
import { ArrowLeft, Box, Loader2, Sparkles } from "lucide-react";
...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("saasBuilder"), alt: "SaaS" },
  { src: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=400&q=80", alt: "Dashboard" },
  { src: "https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=400&q=80", alt: "Product" },
];
...
export default function SaasBuilderPage() {
  ...
  const suggestions = getSuggestionsForFactory("saasBuilder");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "saasBuilder", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI SaaS Builder Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate full SaaS concept: idea, features, pricing, landing sections, dashboard structure, tech stack.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">SaaS idea or description</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. B2B SaaS product, features, pricing tiers"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="saasBuilder"
              factoryName="AI SaaS Builder Factory"
              prompt={prompt}
              resultData={result}
