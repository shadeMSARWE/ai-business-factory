...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("productGenerator"), alt: "Product" },
  { src: "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=400&q=80", alt: "Ecommerce" },
  { src: "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=400&q=80", alt: "Goods" },
];
...
export default function ProductGeneratorPage() {
  ...
  const suggestions = getSuggestionsForFactory("productGenerator");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "productGenerator", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Create product ideas for ecommerce: ideas, brand names, descriptions, packaging concept, pricing strategy.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Product category or niche</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Fashion product ideas, brand names"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="productGenerator"
              factoryName="AI Product Generator"
              prompt={prompt}
              resultData={result}
