...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("storeBuilder"), alt: "Store" },
  { src: "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=400&q=80", alt: "Ecommerce" },
  { src: "https://images.unsplash.com/photo-1556742111-a301076d63d3?w=400&q=80", alt: "Shop" },
];
...
export default function StoreBuilderPage() {
  ...
  const suggestions = getSuggestionsForFactory("storeBuilder");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "storeBuilder", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Store Builder Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Create a ready-to-launch ecommerce store with name, categories, products, and branding.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Store type or description</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Fashion store, clothing, accessories"
        ...
        {!loading && result != null && (
          ...
          >
            ...
            <ResultToolbar
              factoryId="storeBuilder"
              factoryName="AI Store Builder Factory"
              prompt={prompt}
              resultData={result}
