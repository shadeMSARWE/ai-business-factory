...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("videoFactory"), alt: "Video" },
  { src: "https://images.unsplash.com/photo-1611162616305-c69b3fa7fbe0?w=400&q=80", alt: "Short" },
  { src: "https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=400&q=80", alt: "Content" },
];
...
export default function VideoFactoryPage() {
  ...
  const suggestions = getSuggestionsForFactory("videoFactory");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "videoFactory", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <p className="text-slate-400 mb-8">
          Generate viral video concepts: script, scenes, AI image prompts, voiceover, captions, hashtags for TikTok, YouTube Shorts, Reels.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Video topic or platform</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. TikTok viral video, script, scenes, hashtags"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="videoFactory"
              factoryName="AI Video Factory"
              prompt={prompt}
              resultData={result}
