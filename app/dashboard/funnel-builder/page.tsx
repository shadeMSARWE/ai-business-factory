...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("funnelBuilder"), alt: "Funnel" },
  { src: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&q=80", alt: "Conversion" },
  { src: "https://images.unsplash.com/photo-1467232004584-a241de8bcf5d?w=400&q=80", alt: "Landing" },
];
...
export default function FunnelBuilderPage() {
  ...
  const suggestions = getSuggestionsForFactory("funnelBuilder");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "funnelBuilder", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Funnel Builder Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate marketing funnels: structure, landing pages, email sequences, conversion strategy.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Funnel type or goal</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Lead generation funnel, landing, email"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="funnelBuilder"
              factoryName="AI Funnel Builder Factory"
              prompt={prompt}
              resultData={result}
