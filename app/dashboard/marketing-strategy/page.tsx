...
import { ArrowLeft, Megaphone, Loader2, Sparkles } from "lucide-react";
...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("marketingStrategy"), alt: "Strategy" },
  { src: "https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=400&q=80", alt: "Marketing" },
  { src: "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=400&q=80", alt: "Growth" },
];
...
export default function MarketingStrategyPage() {
  ...
  const suggestions = getSuggestionsForFactory("marketingStrategy");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "marketingStrategy", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Marketing Strategy Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate complete marketing strategies: target audience, funnel, ad strategy, content plan, growth tactics.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Business or campaign focus</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. B2B marketing strategy, funnel, content plan"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="marketingStrategy"
              factoryName="AI Marketing Strategy Factory"
              prompt={prompt}
              resultData={result}
