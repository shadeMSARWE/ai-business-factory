...
const PREVIEW_IMAGES = [
  { src: getPreviewImage("contentCalendar"), alt: "Calendar" },
  { src: "https://images.unsplash.com/photo-1506784365847-bbad939e9335?w=400&q=80", alt: "Schedule" },
  { src: "https://images.unsplash.com/photo-1611162616305-c69b3fa7fbe0?w=400&q=80", alt: "Content" },
];
...
export default function ContentCalendarPage() {
  ...
  const suggestions = getSuggestionsForFactory("contentCalendar");
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/factory/run", {
        ...
        body: JSON.stringify({ factoryId: "contentCalendar", prompt: prompt.trim() }),
      });
    ...
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        ...
        <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
          ...
          AI Content Calendar Factory
        </h1>
        <p className="text-slate-400 mb-8">
          Generate social media content calendars: weekly, monthly, platform-specific posts, captions, hashtags.
        </p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 mb-8">
          ...
          <div>
            <Label className="text-slate-400">Platform or timeframe</Label>
            <Input
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g. Weekly content calendar, 7 days"
        ...
        {!loading && result != null && (
          <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <ResultToolbar
              factoryId="contentCalendar"
              factoryName="AI Content Calendar Factory"
              prompt={prompt}
              resultData={result}
