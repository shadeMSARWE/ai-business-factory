...
import { getWebsiteBySlug } from "./storage";

export interface WebsiteDataForDownload {
  businessName?: string;
  heroTitle?: string;
  heroSubtitle?: string;
  aboutTitle?: string;
...
function buildHtmlFromData(d: WebsiteDataForDownload): string {
...
<head>
  ...
  <title>${d.businessName || "Website"}</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .btn{display:inline-block;padding:0.75rem 1.5rem;background:${d.primaryColor || "#8b5cf6"};color:#fff;border:none;border-radius:8px;cursor:pointer;text-decoration:none}
...
<body>
  <header class="hero">
    <div>
      <h1>${d.heroTitle || "Welcome"}</h1>
      <p style="margin-bottom:1.5rem">${d.heroSubtitle || ""}</p>
      <a href="#contact" class="btn">Get Started</a>
    </div>
  </header>
  <section class="section">
    <h2>${d.aboutTitle || "About Us"}</h2>
    <p style="margin-top:1rem">${d.aboutContent || ""}</p>
  </section>
  <section class="section">
    <h2>Our Services</h2>
    <div class="services">
  ...
  <section id="contact" class="section contact">
    <h2>Contact Us</h2>
    <p>${d.contactInfo?.address || ""}</p>
    <p>${d.contactInfo?.phone || ""}</p>
    <p>${d.contactInfo?.email || ""}</p>
    <form onsubmit="event.preventDefault();this.querySelector('button').textContent='Message sent!';this.reset()">
      <input type="text" placeholder="Name" required>
      <input type="email" placeholder="Email" required>
      <textarea placeholder="Message" rows="4" required></textarea>
      <button type="submit" class="btn">Send Message</button>
    </form>
    <div style="margin-top:2rem;height:256px;background:#f0f0f0;border-radius:12px;display:flex;align-items:center;justify-content:center;color:#666">Google Maps placeholder</div>
  </section>
  <footer style="background:#1a1a1a;color:#fff;padding:2rem;text-align:center">
    <p style="font-weight:600">${d.businessName || "Business"}</p>
    <p style="font-size:0.875rem;color:#999;margin-top:0.5rem">© ${new Date().getFullYear()} All rights reserved.</p>
  </footer>
...
export async function downloadWebsiteFromData(data: WebsiteDataForDownload, filename = "website"): Promise<void> {
